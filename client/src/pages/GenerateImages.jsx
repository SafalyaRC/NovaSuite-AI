...
function GenerateImages() {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Generate an image of ${input} in the selected style ${selectedStyle}`;
  ...
  return (
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700 cursor-pointer">
      {/* left col. */}
      <form
      ...
      >
        ...
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {imageStyle.map((item) => (
            <span
              className={`text-x px-4 py-1 border rounded-full cursor-pointer ${
                selectedStyle === item
